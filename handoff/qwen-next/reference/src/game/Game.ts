...
import { PreparedAssets, emptyAssets } from "./skinned/assets";
...
export interface GameOptions {
  ...
  /** fired once per decided match (server-side leaderboard) */
  onMatchEnd?(r: MatchResult): void;
}

/** Result payload persisted through `/api/matches`. */
...
/**
 * Skinned first, procedural as fallback: the dog and King Croak use their imported GLBs when the
 * async preparation step delivered them, otherwise the procedural `CharacterRig` keeps the match
 * playable (and the migration reversible).
 */
function makeRig(id: CharId, assets: PreparedAssets): CharacterRig {
  const gltf = assets.fighters[id];
  const binding = skinnedBindingFor(id);
  if (gltf && binding) {
    try {
      return new SkinnedRig(binding, gltf, FIGHTERS[id].walkSpeed);
    } catch (err) {
      console.warn(`[game] skinned rig failed for ${id}, using procedural fallback`, err);
    }
  }
  if (id === "chicken") return new RobotChicken();
  if (id === "buffalo") return new FluffyBuffalo();
  if (id === "toad") return new KingCroak();
  if (id === "dog") return new Dog();
  return new ScrapEwe();
}
...
export class Game {
  ...
  private assets: PreparedAssets = emptyAssets();
  ...
  constructor(canvas: HTMLCanvasElement, opts: GameOptions) {
    this.opts = opts;
    this.canvas = canvas;
    this.assets = opts.assets ?? emptyAssets();
    ...
    this.arena = buildArena(opts.arena, this.assets);
    ...
    const f1 = new Fighter(FIGHTERS[opts.p1], makeRig(opts.p1, this.assets));
    const f2 = new Fighter(FIGHTERS[opts.p2], makeRig(opts.p2, this.assets));
    this.fighters = [f1, f2];
  ...
  private afterRound() {
    ...
    if (champ) {
      ...
      this.reportMatch(champ, champ === a ? b : a);
      this.emitHud();
    } else {
      this.startRound();
    }
  ...
  /** Hand the decided match to the host app (persisted through /api/matches). */
  private reportMatch(champ: Fighter, loser: Fighter) {
    if (this.matchReported || this.opts.mode === "attract") return;
    ...
    this.opts.onMatchEnd?.({
      ...
      winner: champ.cfg.id,
      loser: loser.cfg.id,
      winnerRounds: champ.roundsWon,
      loserRounds: loser.roundsWon,
      winnerHpPct: Math.round((champ.hp / champ.cfg.maxHp) * 100),
      perfect: !!champ.perfect && champ.hp === champ.cfg.maxHp,
      durationSec: Math.round((performance.now() - this.matchStart) / 1000),
      skinned: [this.fighters[0].rig instanceof SkinnedRig, this.fighters[1].rig instanceof SkinnedRig],
  ...
  private onLand(f: Fighter, impact: number) {
    ...
    // cosmetic kick-up at the landing spot (petals in Kyoto, nothing elsewhere)
    this.arena.footfall?.(f.pos.x, 0, Math.min(1, impact / 13), this.fx);
    if (impact > 9) {
    ...
    if (w && !f.fxDone && f.stateTime >= w.start) {
      ...
      switch (m.fx) {
        ...
        case "flop":
          ...
          break;
        case "spin":
          // Whirlwind Axes: two sweeping arcs, one per hit window
          this.fx.slash(p.clone(), f.facing, 0xffd9a0, 1.7, 0.3, f.stateTime < 0.6 ? 0.7 : -0.9);
          this.fx.ring(p.clone(), 0xffb45e, 1.5, 0.28, p.y);
          this.fx.hitSparks(p.clone(), 0xfff0d0, 10, 0.7, f.facing);
          break;
        case "fang":
          this.fx.slash(p.clone(), f.facing, 0xffb45e, 1.25, 0.26, 1.1);
          this.fx.shockDisc(p.clone(), 0xffd9a0, 0.7, 0.18);
          break;
        default:
          break;
      }
  ...
  private updateCamera(raw: number) {
    ...
    if (this.phase === "attract" && env && !env.orbit) {
      // card-based stage: lateral dolly instead of an orbit, so the backdrop never shows an edge
      const sweep = Math.sin(this.time * 0.16) * 0.5 + 0.5;
      target.set(-5.2 + sweep * 10.4, 2.1 + Math.sin(this.time * 0.4) * 0.35, env.minZ + 2.2);
